...
found:
  ...
  // puts a newly created process in the first queue
  p->queue = 1;
  release(&ptable.lock);
...
scheduler(void)
{
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    // check each queue for processes
    for (int j = 1; j <= NQUEUE; j++) {
      ...
      for (int k = 0; k < NPROC; k++) {
        ...
        // this starts from the front of queue j, and checks our all the processes after to everything before
        ...
        if(p->queue == j && p->state == RUNNABLE) {
          ...
          cprintf("process %s %d has consumed %dms in Q%d \n", p->name, p->pid, j * 10, j); // multiply by 10 for 10ms
      ...
      if (didRun) break; // would continue to search through next queue in MLFQ for next runnable process if no break.
    }
    release(&ptable.lock);
  }
}
